...
def load_excel_data(file):
    ...
    dataframes = [df_productos, df_traspasos, df_ventas]
    ...
    if opcion == "Análisis":
        ...
        if file:
            try:
                ...
                # --- Filtro de temporada ---
                if isinstance(df_ventas, pd.DataFrame) and "Temporada" in df_ventas.columns:
        ...
        # Import prediction functions solo si existen
        show_pred_interface_exists = False
        try:
            from dashboard import show_prediction_interface
            show_pred_interface_exists = True
        except ImportError:
            pass
        ...
        if os.path.exists(training_data_path):
            try:
                ...
                if show_pred_interface_exists:
                    show_prediction_interface(df_training)
                else:
                    st.error("No se encontró la función show_prediction_interface en dashboard.py")
